...
const selectOperationsSliceState = (state: RootStateType) => state.operations

const selectExpensesByMonthAndYear = (date: number) =>
    createSelector([selectOperationsSliceState], ({ entities }) => {
        return Object.values(entities).filter(
            (operation) => getYear(operation.date) === getYear(date) && getMonth(operation.date) === getMonth(date),
        )
    })

const selectTopExpensesByMonthAndYear = (top: number, date: number) =>
    createSelector([selectOperationsSliceState], ({ entities }) => {
        const operationsByMonthAndYear = Object.values(entities).filter(
            (operation) =>
...
export const operationsSelectors = {
    ...
    selectTopExpensesByMonthAndYear,
}
